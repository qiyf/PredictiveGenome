...
!
    ...
    !double precision, parameter :: r_cut=1.5, sigma=3.72
    integer, parameter:: nCS=26                         ! number of chromatin types
                                                        ! 26 is the 'mix' type
    integer           :: nIdeal, nCV, nCG, cgfactor, atomSta, atomStap1,    &
                         atomEnd, first_frame

    ! optimization variables
    double precision :: dx, dy, dz, rij, r2, pij, cgmean, r_cut4
    integer,allocatable:: chromatinState(:)
    double precision, allocatable :: cmap(:,:),       &   ! per bead
                                     cmapCS(:,:),     &   ! per chromatin state
    ...
    ! general variables
    integer           :: ib, jb, ifr, iCS,jCS, gpos, iCV, kc, lc
    ...
    !!
    !! save the contact matrix
    !open(unit=10, file='contact_map.txt')
    !do ib = 1, natom
        !do jb = 1, natom-1
            !write(10, '(f12.8)', advance='no') cmap(ib, jb)
        !enddo
        !write(10, '(f12.8)') cmap(ib,natom)
    !enddo
    !close(10)
